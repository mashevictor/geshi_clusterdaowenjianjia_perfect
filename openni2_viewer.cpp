    ...
    while (!cloud_viewer_->wasStopped () && (image_viewer_ && !image_viewer_->wasStopped ()))
    {
      ...
      if (cloud)
      {
        ...
        if (!cloud_viewer_->updatePointCloud (cloud, "OpenNICloud"))
        {
            ...
            std::cerr << "PointCloud representing the cloud_rest component: " << cloud_rest->width * cloud_rest->height << " data points." << std::endl;
            extract.setInputCloud (cloud_rest);
            extract.setIndices (inliers);
            extract.setNegative (false);
            extract.filter (*cloud_abovePlane);
            filtered.swap (cloud_abovePlane);
            pcl::visualization::PointCloudColorHandlerCustom<PointType> yellow(cloud_abovePlane, 0, 255, 0);
